...
class nnUNetPredictor(object):
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                if ofile is not None:
                    ...
                    #    f.write(os.path.basename(ofile) + "\n")
                else:
        ...
        torch.save(self.feature_test,self.feature_test_savepath)
        return ret
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        _,feature,prediction = self.network(x,self.feature,self.usefeature)
        self.token_save(feature,0)
        #self.token_save(feature, 0)
        ...
        if mirror_axes is not None:
            ...
            if 0 in mirror_axes:
                _,feature,pre=self.network(torch.flip(x, (2,)),self.feature,self.usefeature)
                self.token_save(feature,0)
                #pre = self.network(torch.flip(x, (2,)))
                prediction += torch.flip(pre, (2,))
            if 1 in mirror_axes:
                _,feature, pre= self.network(torch.flip(x, (3,)),self.feature,self.usefeature)
                self.token_save(feature,0)
                #pre = self.network(torch.flip(x, (3,)))
                prediction += torch.flip(pre, (3,))
            if 2 in mirror_axes:
                _,feature, pre= self.network(torch.flip(x, (4,)),self.feature,self.usefeature)
                self.token_save(feature,0)
                #pre = self.network(torch.flip(x, (4,)))
                prediction += torch.flip(pre, (4,))
            if 0 in mirror_axes and 1 in mirror_axes:
                _,feature, pre= self.network(torch.flip(x, (2,3)),self.feature,self.usefeature)
                self.token_save(feature,0)
                #pre = self.network(torch.flip(x, (2, 3)))
                prediction += torch.flip(pre, (2, 3))
            if 0 in mirror_axes and 2 in mirror_axes:
                _,feature, pre= self.network(torch.flip(x, (2, 4)),self.feature,self.usefeature)
                self.token_save(feature,0)
                #pre = self.network(torch.flip(x, (2, 4)))
                prediction += torch.flip(pre, (2, 4))
            if 1 in mirror_axes and 2 in mirror_axes:
                _,feature, pre= self.network(torch.flip(x, (3, 4)),self.feature,self.usefeature)
                self.token_save(feature,0)
                #pre = self.network(torch.flip(x, (3, 4)))
                prediction += torch.flip(pre, (3, 4))
            if 0 in mirror_axes and 1 in mirror_axes and 2 in mirror_axes:
                _,feature, pre= self.network(torch.flip(x, (2, 3,4)),self.feature,self.usefeature)
                self.token_save(feature,0)
                #pre = self.network(torch.flip(x, (2, 3, 4)))
                prediction += torch.flip(pre, (2, 3, 4))
    ...
    def token_save(self,feature,batch_i):
        if self.feature_test == None:
            ...
            self.feature_test = cropfeature
        else:
            for i in range(len(feature)):
                fi = feature[i]

                t = fi[batch_i,:, :].detach()
                self.feature_test[i] = torch.cat((self.feature_test[i], t))
        return True
